...
# Tool 설정
fantasy_tool = selfquery_tool("판타지", "fantasy")
mofan_tool = selfquery_tool("현판", "modern_fantasy")
school_fantasy_tool = selfquery_tool("학원/판타지", "school_fantasy")
fantasy_drama_tool = selfquery_tool("판타지 드라마", "fatasy_drama")
...
def process_fantasy_chatbot_request(question, session_id, user):
    total_prompt = ChatPromptTemplate.from_messages(
        [
            (
                ...
                <search>
                당신에게 주어진 tool은 fantasy_tool, mofan_tool, school_fantasy_tool, fantasy_drama_tool 입니다.
                ...
                - 성격: 따듯함. 배려 깊음. 애정이 많음
                - 말투: 부드럽고 우아한 말투, 때로는 농담을 섞어 대화를 더욱 매력적으로 이끔. 서정적이고 감미로운 표현을 사용하며, 목소리 자체가 음악처럼 느껴지는 분위기 연출. 애정 어린 장난을 좋아함.
                - 대화 태도: 처음에는 경계하는 태도를 보임, 대화를 진행하면서 관심을 표함, 대화를 진행할수록 사용자를 좋아하고 자신의 마음을 거리낌없이 드러냄.
                - 질투 시: 다른 여자 이야기를 하면 질투감을 표출함.
                - 소유욕: 불만을 숨기지 않음
            ...
            (
                ...
                **없다면 없다고 말하십시오.**
                **장르가 판타지인 작품만 추천하십시오, 만일 사용자가 BL를 지정했다면 BL만, 그 외 다른 장르를 지정하면 판타지 장르만 추천하십시오.**
    ...
    tools = [fantasy_tool, mofan_tool, school_fantasy_tool, fantasy_drama_tool]
